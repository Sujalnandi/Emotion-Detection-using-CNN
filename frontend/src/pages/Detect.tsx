...
import { predictEmotion, predictRealtimeBase64, type FacePrediction, type PredictionResponse } from "../utils/api";
...
export default function Detect() {
  ...
  const captureAndPredict = async () => {
    if (!videoRef.current || !canvasRef.current || realtimeBusyRef.current) return;
  ...
  const toggleRealtime = () => {
    ...
    captureAndPredict();
    intervalRef.current = window.setInterval(captureAndPredict, 100);
  };
  ...
  const uploadFaces = result?.faces ?? [];

  const resolveTop3 = (face: FacePrediction): Array<{ emotion: string; confidence: number }> => {
    if ((face.top3 ?? []).length > 0) return (face.top3 ?? []).slice(0, 3);
  ...
  const confidenceBand = (confidence: number): "high" | "medium" | "low" => {
  ...
  const confidenceBadgeClass = (confidence: number): string => {
    const band = confidenceBand(confidence);
    if (band === "high") return "border-emerald-300/40 bg-emerald-400/15 text-emerald-200";
    if (band === "medium") return "border-amber-300/40 bg-amber-400/15 text-amber-200";
    return "border-rose-300/40 bg-rose-400/15 text-rose-200";
  };

  const confidenceBarClass = (confidence: number): string => {
    const band = confidenceBand(confidence);
    if (band === "high") return "bg-gradient-to-r from-emerald-500 to-lime-300";
    if (band === "medium") return "bg-gradient-to-r from-amber-500 to-yellow-300";
    return "bg-gradient-to-r from-rose-500 to-red-300";
  };

  const overlayLabel = (face: FacePrediction): string => {
    if (face.displayLabel) return face.displayLabel;
    if (face.confidence < 0.4) {
      return `${formatEmotion(face.emotion)} (low confidence ${Math.round(face.confidence * 100)}%)`;
    }
    return `${formatEmotion(face.emotion)} (${Math.round(face.confidence * 100)}%)`;
  };

  const resultTop3 = result?.top3 ?? probabilityEntries.slice(0, 3).map(([emotion, confidence]) => ({ emotion, confidence }));
  const realtimeTop3 =
    realtimeResult?.top3 ?? realtimeProbabilityEntries.slice(0, 3).map(([emotion, confidence]) => ({ emotion, confidence }));
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-white/10 bg-slate-900/60">
              {previewUrl ? (
                <div className="relative h-64 w-full">
                  <img src={previewUrl} alt="Selected file preview" className="h-full w-full object-fill" />

                  {uploadFaces.map((face) => {
                    ...
                    return (
                      <div
                        key={`upload-face-${face.id}`}
                        className="absolute"
                        style={{ left: `${x}%`, top: `${y}%`, width: `${w}%`, height: `${h}%` }}
                      >
                        <div
                          className="h-full w-full rounded-md border-2 shadow-[0_0_20px_rgba(15,23,42,0.45)]"
                          style={{ borderColor: color }}
                        />
                        <div
                          className="absolute -top-6 left-0 rounded px-2 py-0.5 text-[11px] text-slate-900"
              ...
              >
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`rounded-full border px-2 py-0.5 text-[11px] font-medium uppercase tracking-[0.12em] ${confidenceBadgeClass(result.confidence)}`}
                    >
                      {confidenceBand(result.confidence)}
                    </span>
                  </div>
                  <p className="mt-2 text-2xl font-bold text-white">{result.displayLabel ?? formatEmotion(result.emotion)}</p>
                </div>

                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Confidence</p>
                  <p className="mt-2 text-2xl font-bold text-white">{(result.confidence * 100).toFixed(2)}%</p>
                  <div className="mt-3 h-2.5 rounded-full bg-white/10">
                    <div
                      className={`h-2.5 rounded-full ${confidenceBarClass(result.confidence)}`}
                ...
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Top 3 Predictions</p>
                  <div className="mt-3 space-y-2">
                    {resultTop3.length === 0 ? (
                      <p className="text-sm text-slate-400">No ranked predictions returned.</p>
                    ) : (
                      resultTop3.map((item, idx) => (
                        <div key={`${item.emotion}-${idx}`} className="rounded-lg border border-white/10 bg-slate-950/40 p-2.5">
                          <div className="flex items-center justify-between text-sm text-slate-200">
                            <span className={idx === 0 ? "font-semibold text-white" : "text-slate-300"}>{formatEmotion(item.emotion)}</span>
                            <span>{(item.confidence * 100).toFixed(1)}%</span>
                          </div>
                          <div className="mt-2 h-2 rounded-full bg-white/10">
                            <div
                              className={`h-2 rounded-full ${confidenceBarClass(item.confidence)}`}
                              style={{ width: `${Math.max(2, Math.min(100, item.confidence * 100))}%` }}
                            />
                          </div>
                        </div>
                      ))
                      ...
                      probabilityEntries.map(([emotion, score]) => (
                        <div key={emotion}>
                          ...
                          <div className="mt-1 h-2 rounded-full bg-white/10">
                            <div
                              className={`h-2 rounded-full ${confidenceBarClass(score)}`}
                ...
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Per-Face Results</p>
                  <div className="mt-3 space-y-3">
                    {uploadFaces.length === 0 ? (
                      <p className="text-sm text-slate-400">No face-level details returned.</p>
                    ) : (
                      uploadFaces.map((face) => {
                        const faceTop3 = resolveTop3(face);
                        return (
                          <div key={`upload-result-face-${face.id}`} className="rounded-xl border border-white/10 bg-slate-950/45 p-3">
                            <div className="flex items-center justify-between gap-2">
                              <p className="text-sm font-semibold text-white">Face #{face.id}: {overlayLabel(face)}</p>
                              <span className={`rounded-full border px-2 py-0.5 text-[11px] uppercase ${confidenceBadgeClass(face.confidence)}`}>
                                {confidenceBand(face.confidence)}
                              </span>
                            </div>
                            <div className="mt-2 space-y-2">
                              {faceTop3.map((item, idx) => (
                                <div key={`upload-face-${face.id}-rank-${idx}`} className="text-xs text-slate-300">
                                  <div className="flex items-center justify-between">
                                    <span className={idx === 0 ? "font-semibold text-white" : ""}>{formatEmotion(item.emotion)}</span>
                                    <span>{(item.confidence * 100).toFixed(1)}%</span>
                                  </div>
                                  <div className="mt-1 h-2 rounded-full bg-white/10">
                                    <div
        ...
        <Card>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-300">Capture up to 10 FPS and stream base64 frames to /realtime.</p>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1.35fr_1fr]">
            <div className="relative overflow-hidden rounded-xl border border-white/10 bg-slate-900/70">
              <video ref={videoRef} autoPlay playsInline muted className="h-[340px] w-full object-fill" />
              ...
              {(realtimeResult?.faces ?? []).map((face) => {
                ...
                return (
                  <div
                    ...
                    style={{ left: `${x}%`, top: `${y}%`, width: `${w}%`, height: `${h}%` }}
                  >
                    <div className="h-full w-full rounded-md border-2 shadow-[0_0_20px_rgba(15,23,42,0.45)]" style={{ borderColor: color }} />
                    <div className="absolute -top-6 left-0 rounded px-2 py-0.5 text-[11px] text-slate-900" style={{ backgroundColor: color }}>
                      {overlayLabel(face)}
                    </div>
                  </div>
                );
                ...
                <div className="mt-3 space-y-3">
                  <div className="rounded-lg border border-white/10 bg-slate-950/45 p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`rounded-full border px-2 py-0.5 text-[11px] uppercase ${confidenceBadgeClass(realtimeResult.confidence)}`}>
                        {confidenceBand(realtimeResult.confidence)}
                      </span>
                    </div>
                    <p className="mt-1 text-xl font-semibold text-white">{realtimeResult.displayLabel ?? formatEmotion(realtimeResult.emotion)}</p>
                    <p className="text-sm text-slate-200">Confidence: {(realtimeResult.confidence * 100).toFixed(1)}%</p>
                  </div>

                  <div className="rounded-lg border border-white/10 bg-slate-950/45 p-3">
                    <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Top 3 Predictions</p>
                    <div className="mt-2 space-y-2">
                      {realtimeTop3.map((item, idx) => (
                        <div key={`realtime-top3-${item.emotion}-${idx}`} className="text-xs text-slate-300">
                          <div className="flex items-center justify-between">
                            <span className={idx === 0 ? "font-semibold text-white" : ""}>{formatEmotion(item.emotion)}</span>
                            <span>{(item.confidence * 100).toFixed(1)}%</span>
                          </div>
                  ...
                  <div className="space-y-2">
                    {(realtimeResult.faces ?? []).length === 0 ? (
                      <p className="text-sm text-slate-400">No face-level results yet.</p>
                    ) : (
                      realtimeResult.faces.map((face) => {
                        const faceTop3 = resolveTop3(face);
                        return (
                          <div key={`live-face-card-${face.id}`} className="rounded-lg border border-white/10 bg-slate-950/45 p-3">
                            <div className="flex items-center justify-between gap-2">
                              <p className="text-sm font-semibold text-white">Face #{face.id}: {overlayLabel(face)}</p>
                              <span className={`rounded-full border px-2 py-0.5 text-[11px] uppercase ${confidenceBadgeClass(face.confidence)}`}>
                                {confidenceBand(face.confidence)}
                            ...
                            <div className="mt-2 space-y-2">
                              {faceTop3.map((item, idx) => (
                                <div key={`live-face-${face.id}-rank-${idx}`} className="text-xs text-slate-300">
                                  <div className="flex items-center justify-between">
                                    <span className={idx === 0 ? "font-semibold text-white" : ""}>{formatEmotion(item.emotion)}</span>
                                    <span>{(item.confidence * 100).toFixed(1)}%</span>
                                  </div>
