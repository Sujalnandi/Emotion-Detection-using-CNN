...
def _public_prediction(result: dict[str, Any]) -> PredictionResponse:
    ...
    if not probabilities_raw and len(faces) > 0:
        probabilities_raw = faces[0].get("probabilities") or {}

    emotion = str(result.get("emotion", "")).strip().lower()
    confidence = float(result.get("confidence", 0.0) or 0.0)
    if emotion in {"", "no face"}:
        top_face = max(faces, key=lambda item: float(item.get("confidence", 0.0) or 0.0))
        emotion = str(top_face.get("emotion", "unknown")).lower()
        confidence = float(top_face.get("confidence", 0.0) or 0.0)
    ...
    emotion_title = str(emotion).capitalize()
    display_label = (
        f"{emotion_title} (low confidence {confidence * 100.0:.0f}%)"
        if confidence < 0.4
        else emotion_title
    )

    top3_raw = list(result.get("top3") or [])
    if not top3_raw and probabilities:
        ranked = sorted(probabilities.items(), key=lambda item: item[1], reverse=True)[:3]
        top3_raw = [{"emotion": key, "confidence": value} for key, value in ranked]
    ...
    if not boxes:
        boxes = [face.get("bbox", {}) for face in faces if isinstance(face, dict)]
